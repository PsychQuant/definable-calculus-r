...
test_that("sqrt(x^2) folds to abs(x) only when abs is in the L_0 catalog", {
  on.exit(dat:::register_default_catalog(), add = TRUE)
  # (a) with abs registered -> abs(v), NOT v
  extend_language("L_0", "abs", function(expr, var) {
    bquote(sign(.(expr[[2L]])) * .(dat:::.grad_expr(expr[[2L]], var)))
  })
  expect_identical(dat:::.algebraic_simplify(quote(sqrt(v^2))), quote(abs(v)))
  # (b) negative-input correctness: |x|, not x (the wrong fold)
  folded <- dat:::.algebraic_simplify(quote(sqrt(v^2)))
...
test_that("sqrt(x^2) is a no-op when abs is not registered", {
  ...
  expect_identical(dat:::.algebraic_simplify(quote(sqrt(v^2))), quote(sqrt(v^2)))
})

# ===== Group 2 (RED) — constant folding ==============================
test_that("literal-only arithmetic folds to a single literal", {
  expect_equal(dat:::.algebraic_simplify(quote(2 + 3)), 5)
})

test_that("literal factors fold while the variable term is preserved", {
  folded <- dat:::.algebraic_simplify(quote(2 * sum(v^2) * 4))
  v <- c(1, 2, 3)
...
test_that("non-finite literal arithmetic is NOT folded (no Inf in AST)", {
  expect_identical(dat:::.algebraic_simplify(quote(1 / 0)), quote(1 / 0))
})

test_that("constant folding is idempotent", {
  once  <- dat:::.algebraic_simplify(quote(2 * sum(v^2) * 4))
  twice <- dat:::.algebraic_simplify(once)
  expect_identical(twice, once)
})

# ===== Group 3 (RED) — conservative trig identities ==================
test_that("sin^2 + cos^2 folds to 1 (both orders)", {
  expect_identical(dat:::.algebraic_simplify(quote(sin(v)^2 + cos(v)^2)), 1)
  expect_identical(dat:::.algebraic_simplify(quote(cos(v)^2 + sin(v)^2)), 1)
})

test_that("2 sin cos folds to sin(2x)", {
  expect_identical(dat:::.algebraic_simplify(quote(2 * sin(v) * cos(v))), quote(sin(2 * v)))
})

test_that("trig identity does NOT fold when inner arguments differ", {
  e <- quote(sin(v)^2 + cos(w)^2)
  expect_identical(dat:::.algebraic_simplify(e), e)
})
...
test_that(".substitute_sum_sq keeps k=2 on fast_sum_sq and routes k>=3 to fast_sum_pow", {
  expect_identical(dat:::.substitute_sum_sq(quote(sum(v^2)), "v"), quote(fast_sum_sq(v)))
  expect_identical(dat:::.substitute_sum_sq(quote(sum(v^4)), "v"), quote(fast_sum_pow(v, 4)))
})
...
test_that("non-generalizable powers fall through unchanged", {
  expect_identical(dat:::.substitute_sum_sq(quote(sum(v^2.5)), "v"), quote(sum(v^2.5)))
  expect_identical(dat:::.substitute_sum_sq(quote(sum(v^1)), "v"), quote(sum(v^1)))
  expect_identical(dat:::.substitute_sum_sq(quote(sum((v + 1)^3)), "v"), quote(sum((v + 1)^3)))
})
